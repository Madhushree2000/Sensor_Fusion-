...
def main():
    ...
    # Reduced noise and correlation for improved precision
# Increase correlations and slightly relax JCBB alphas
# High noise, strong correlations, and strict JCBB alphas
    # sigmas = 0.025 * np.array([0.00015, 0.00007, 8 * np.pi / 180])
    # CorrCoeff = np.array([[1, 0, 0], [0, 1, 0.99], [0, 0.99, 1]])
    # Q = np.diag(sigmas) @ CorrCoeff @ np.diag(sigmas)
#     Q = np.array([
#     [0.0000055, 0, 0],
#     [0, 0.00000275, 0.0000882],
#     [0, 0.0000882, 0.0031416]
# ])**2
#     R = np.diag([0.080, 0.0150]) ** 2
#     JCBBalphas = np.array([10**-2, 10**-3])  # Very strict associations
    ...
    R = np.diag([0.085, 0.0160]) ** 2
    JCBBalphas = np.array([10**-1, 10**-2])  # Relaxed thresholds
    ...
    # %% Final Consistency Evaluation for NIS
    total_NIS = NISnorm[:mk]
    avg_NIS = np.mean(total_NIS)
    confidence_bounds = chi2.interval(confidence_prob, 2)  # Degrees of freedom = 2 for 2D measurements
    confidence_bounds = np.array(confidence_bounds) / 2  # Normalize bounds

    print(f"Average NIS over the run: {avg_NIS:.3f}")
    print(f"95% Confidence interval bounds: {confidence_bounds}")
