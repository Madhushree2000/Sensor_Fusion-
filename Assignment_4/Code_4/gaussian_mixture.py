...
class GaussianMixture(Generic[S]):
    ...
    def mean(self):
        ...
        mean = np.zeros(self.gaussians[0].mean.shape)
    ...
    def cov(self):
        ...
        cov_tilde = np.zeros(self.gaussians[0].mean.shape)
        cov = np.zeros(self.gaussians[0].cov.shape)

        for i in range(len(self.weights)):
            cov_tilde += self.weights[i] * (self.gaussians[i].mean) @ (self.gaussians[i].mean).T 

        cov_tilde -= self.mean() @ self.mean().T
